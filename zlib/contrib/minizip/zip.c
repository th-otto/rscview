...
#ifndef Z_BUFSIZE
#define Z_BUFSIZE 0x10000UL /* (16384) */
#endif
...
#define ENDHEADERMAGIC      (0x06054b50UL)
#define ZIP64ENDHEADERMAGIC      (0x6064b50UL)
#define ZIP64ENDLOCHEADERMAGIC   (0x7064b50UL)
...
local ZPOS64_T zip64local_SearchCentralDir64(const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream)
{
  ...
  if (uL != 0x06064b50UL) /* signature of 'Zip64 end of central directory' */
    return 0;
...
static int Write_LocalFileHeader(zip64_internal* zi, const char* filename, uInt size_extrafield_local, const void* extrafield_local)
{
  ...
  {
    if(zi->ci.zip64)
      err = zip64local_putValue(&zi->z_filefunc,zi->filestream,(uLong)0xFFFFFFFFUL,4); /* compressed size, unknown */
    else
  ...
  {
    if(zi->ci.zip64)
      err = zip64local_putValue(&zi->z_filefunc,zi->filestream,(uLong)0xFFFFFFFFUL,4); /* uncompressed size, unknown */
    else
...
#    endif
    ...
    if(zi->ci.pos_local_header >= 0xffffffffUL)
      zip64local_putValue_inmemory(zi->ci.central_header+42,(uLong)0xffffffffUL,4);
    else
...
extern int ZEXPORT zipCloseFileInZipRaw64 (zipFile file, ZPOS64_T uncompressed_size, uLong crc32)
{
    ...
    uLong invalidValue = 0xffffffffUL;
    short datasize = 0;
    int err=ZIP_OK;
...
#    endif
    ...
    if(compressed_size >= 0xffffffffUL || uncompressed_size >= 0xffffffffUL || zi->ci.pos_local_header >= 0xffffffffUL)
    {
      /*version Made by*/
    ...
    if(compressed_size >= 0xffffffffUL)
    ...
    if (err==ZIP_OK)
    {
        ...
        if(uncompressed_size >= 0xffffffffUL || compressed_size >= 0xffffffffUL )
        {
...
static int Write_EndOfCentralDirectoryRecord(zip64_internal* zi, uLong size_centraldir, ZPOS64_T centraldir_pos_inzip)
{
  ...
  if (err==ZIP_OK) /* offset of start of central directory with respect to the starting disk number */
  {
    ...
    if(pos >= 0xffffffffUL)
    {
      err = zip64local_putValue(&zi->z_filefunc,zi->filestream, (uLong)0xffffffffUL,4);
    }
    else
