...
typedef struct {
	BIG_IMAGE big_off_mask;
	BIG_IMAGE big_off_data;
	BIG_IMAGE big_on_mask;
	BIG_IMAGE big_on_data;
	SMALL_IMAGE small_off_mask;
	SMALL_IMAGE small_off_data;
	SMALL_IMAGE small_on_mask;
	SMALL_IMAGE small_on_data;
} MAC_IMAGE;
...
static _BOOL get_macimage(_WORD type, _WORD state, _WORD *width, _WORD *height, _UBYTE **mask, _UBYTE **data)
{
	...
	if (ch < 13)
	{
		...
		*height = 8;
		if (state & OS_SELECTED)
		{
			*mask = image->small_on_mask;
			*data = image->small_on_data;
		} else
		{
			*mask = image->small_off_mask;
			*data = image->small_off_data;
		}
	} else
	{
		*width = *height = 16;
		if (state & OS_SELECTED)
		{
			*mask = image->big_on_mask;
			*data = image->big_on_data;
		} else
		{
			*mask = image->big_off_mask;
			*data = image->big_off_data;
		}
	}
	return TRUE;
}
	...
	*draw_3d = color_3d(flags, state, &color, &pattern);
	...
	case MAC_RADIO:
		...
		if (h == 16 || h == 8)
		{
			...
			{
				W_Draw_Icon(od_handle, x, y, *w, h, data, mask, COL_SET, COL_RESET, FALSE);
			}
		} else
#endif
		{
