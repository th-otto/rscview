/*
 *  $Id: vex_curv.c,v 1.7 2003/08/07 07:09:01 a_bercegeay Exp $
 */
...
void
vex_curv (short handle, VEX_CURV newhdlr, VEX_CURV *old)
{
	short vdi_control[VDI_CNTRLMAX]; 

	VDI_PARAMS(vdi_control, 0L, 0L, vdi_dummy, vdi_dummy);
	
	vdi_control_ptr(0, VEX_CURV) = newhdlr;
	
	VDI_TRAP_00 (vdi_params, handle, 127);
	
	if (old)
		*old = vdi_control_ptr(1, VEX_CURV);
}
