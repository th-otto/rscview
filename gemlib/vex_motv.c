/*
 *  $Id: vex_motv.c,v 1.7 2003/08/07 07:09:01 a_bercegeay Exp $
 */
...
void
vex_motv (short handle, VEX_MOTV newhdlr, VEX_MOTV *old)
{
	short vdi_control[VDI_CNTRLMAX]; 

	VDI_PARAMS(vdi_control, 0L, 0L, vdi_dummy, vdi_dummy );
	
	vdi_control_ptr(0, VEX_MOTV) = newhdlr;
	
	VDI_TRAP_00 (vdi_params, handle, 126);
	
	if (old)
		*old = vdi_control_ptr(1, VEX_MOTV);
}
