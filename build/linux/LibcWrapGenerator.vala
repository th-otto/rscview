...
const string DEFAULT_TARGET      = "2.11";
const string DEFAULT_TARGET_HELP = "Target glibc ABI (Default 2.11)";
...
const GLib.DebugKey[] libcwrap_debug_keys = {
	{ "collect",         DF.COLLECT         },
	{ "filter",          DF.FILTER          },
...
public class Main : Object {
	...
	private static void appendSymbols (StringBuilder headerFile, bool unavailableSymbols) {
		...
		sorted_keys.add_all (symbolMap.keys);
		sorted_keys.sort();
		foreach (var it in sorted_keys) {
	...
	private static void generateHeader () throws Error {
		...
		headerFile.append ("#if defined(__linux__) && !defined (__LIBC_CUSTOM_BINDINGS_H__)\n");
		headerFile.append ("\n");
